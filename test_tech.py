def extract_calibration_value(line):
    # Extraire tous les chiffres dans la ligne
    digits = [char for char in line if char.isdigit()]
    # Combiner le premier et le dernier chiffre pour former le nombre
    if digits:
        return int(digits[0] + digits[-1])
    return 0

def calculate_total_calibration(document):
    # Diviser le document en lignes et calculer la somme des valeurs d'étalonnage
    return sum(extract_calibration_value(line) for line in document.splitlines())
...
print("Valeur totale d'étalonnage :", total_calibration)
